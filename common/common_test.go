...
func TestSetOptionalRedisPasswordFromKubeConfig(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedErr:      "",
			secret: &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{Name: DefaultRedisInitialCredentials},
				Data:       map[string][]byte{DefaultRedisInitialCredentialsKey: []byte("password123")},
			},
		},
		{
			...
			namespace:        "default",
			expectedPassword: "",
			expectedErr:      fmt.Sprintf("failed to get secret default/%s", DefaultRedisInitialCredentials),
			secret:           nil,
		},
		{
			...
			namespace:        "default",
			expectedPassword: "",
			expectedErr:      fmt.Sprintf("secret default/%s does not contain key %s", DefaultRedisInitialCredentials, DefaultRedisInitialCredentialsKey),
			secret: &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{Name: DefaultRedisInitialCredentials},
